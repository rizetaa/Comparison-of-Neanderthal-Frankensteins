...
FIGS    = f"{WORKDIR}/results/chr{CHR}/figures"
...
ax.set_title(f"Спектр частот неандертальской интрогрессии\n(chr{CHR}, окна 1 kb)", fontsize=14)
ax.set_yscale("log")
...
ax.set_xlabel(f"Позиция на chr{CHR} (Mb)", fontsize=13)
...
ax2.set_title(f"Boxplot: Fw_bin vs Sw\n(chr{CHR}, Fw > 0)", fontsize=13)
...
plt.suptitle(f"Очищающий отбор: сила eQTL vs частота интрогрессии (chr{CHR})",
             fontsize=14, y=1.02)
plt.tight_layout()
...
for i, cat in enumerate(dtss_cats):
    ...
    try:
        ...
        # seaborn >= 0.13: split не поддерживается
        sns.violinplot(
            data=cat_data, x="dtss_cat", y="Sw_max", hue="group",
            inner="box", ax=ax,
            palette=colors_split, cut=0,
...
plt.suptitle(f"Сравнение силы eQTL: интрогрессия vs контроль\nпо расстоянию до TSS (chr{CHR})",
             fontsize=13, y=1.06)
plt.tight_layout()
...
ax_isfs.set_title(f"A. iSFS (chr{CHR})", fontsize=11, fontweight="bold")
...
ax_man.set_title(f"B. Manhattan plot (chr{CHR})", fontsize=11, fontweight="bold")
ax_man.set_xlabel("Позиция (Mb)", fontsize=9)
ax_man.set_ylabel("Fw", fontsize=9)
ax_man.set_xlim(0, max_pos_mb * 1.05)

# 6c: Boxplot Fw_bin vs Sw
...
# 6d: Scatter адаптивная интрогрессия
...
# 6e: Bootstrap результаты (bar chart p-values)
...
plt.suptitle(f"Сводная панель: eQTL-влияние неандертальской интрогрессии (chr{CHR})",
...
print("  fig3_violin_boxplot.png/pdf    — Violin/Boxplot Fw_bin vs Sw")
print("  fig4_split_violin.png/pdf      — Split Violin по D_TSS")
print("  fig5_scatter_adaptive.png/pdf  — Scatter Fw vs Sw (кандидаты)")
print("  fig6_summary_panel.png/pdf     — Сводная панель")
