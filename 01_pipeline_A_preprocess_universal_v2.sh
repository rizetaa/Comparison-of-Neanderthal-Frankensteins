#!/bin/bash
# Скрипт 01: Pipeline A — Препроцессинг для глобального анализа (универсальный)
# bedtools, python3 (pandas, numpy), tabix
# Использование: CHR=6 bash 01_pipeline_A_preprocess_universal_v2.sh
...
# Сохраняем H для дальнейших шагов
...
print(f"  NIS BED сохранён: {OUT}/nis_segments.bed")
...
# 2b: bedtools intersect: окна × NIS-сегменты
...
echo "  Пересечений окна×NIS: $(wc -l < "$OUT/windows_x_nis.bed")"

# 2c: Вычисляем Fw векторно в Python
python3 << 'PYEOF'
import pandas as pd
import numpy as np
import os
...
if [ ! -f "$EQTL_RAW" ]; then
    ...
    echo "  Запустите сначала: bash scripts/00_prefilter_gtex_v2.sh"
    exit 1
fi
...
# Читаем уже отфильтрованный файл (только chr, ~50-100 МБ)
eqtl = pd.read_csv(raw_path, sep="\t")
print(f"  Загружено eQTL chr{CHR}: {len(eqtl)}")

if len(eqtl) == 0:
    print(f"  ПРЕДУПРЕЖДЕНИЕ: eQTL chr{CHR} не найдены — создаём пустой BED.")
...
# Финальный фильтр chr
eqtl = eqtl[eqtl["vchr"].isin([f"chr{CHR}", str(CHR)])].copy()
print(f"  eQTL на chr{CHR} (финальный): {len(eqtl)}")

# Фильтр: только биаллельные SNP
eqtl = eqtl[
    (eqtl["vref"].str.len() == 1) &
    (eqtl["valt"].str.len() == 1)
].copy()
...
# Сохраняем полный список eQTL chr для Pipeline B
eqtl.to_csv(tsv_path, sep="\t", index=False)
print(f"  Сохранён: {tsv_path}")
...
os.remove(raw_path)
print(f"  Удалён промежуточный: {raw_path}")
PYEOF
...
# Парсим GTF — только строки с feature == "gene" на chr{CHR}
tss_list = []
with open(GTF) as fh:
    for line in fh:
        if line.startswith("#"):
...
print(f"  TSS BED сохранён: {OUT}/chr{CHR}_tss.bed")
PYEOF

echo "[$(date)] Шаг 6 завершён."
...
# Читаем карту рекомбинации HapMap
...
# Векторизованный расчёт средней скорости рекомбинации
...
echo "[$(date)] Шаг 8 завершён."
echo "[$(date)] Pipeline A завершён"
